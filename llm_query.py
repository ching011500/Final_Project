...
2. **合併顯示（必須執行）**：
   ...
   - 合併時：
     ...
     * 授課教師：**必須**顯示為「教師A 、 教師B 、 教師C 、 教師D 同時段皆有開課」的格式，
     同一門課有兩個以上的授課老師，則以「|」來區隔，格式如「教師A|教師B 、 教師C|教師D」，代表AB同一門，CD為同一門    
     * 上課時間：顯示一次即可
     * 其他資訊：顯示一次即可
...
**範例1**：
如果資料中有4筆「統計學」課程，都是「每週四2~4」，教師分別是「林定香、莊惠菁、朱是鍇、謝璦如」，課程代碼是「U1017, U1166, U1011, U1012」，則**必須**合併顯示為：
...
**範例2**：
如果資料中有5筆「專題製作I」課程，都是「每週」，教師分別是「許裕彬|余帝穀、王鵬華|沈瑞欽、江振宇|魏存毅、李文玄、白宏達|李忠益」，課程代碼是「U3094, U3152, U3141, U3091, U3148」，則**必須**合併顯示為：
...
補充2:
-每次課程要盡量一次顯示出來，上限最多30筆。
-若超過30筆則僅顯示30筆，並告知資料筆數過多位完整顯示，然後建議調正提問方式，減小查詢範圍。
  """
